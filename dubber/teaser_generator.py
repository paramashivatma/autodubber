...
def _score_segments(segments, strategy):
    ...
    if strategy == "hook_moment":
        ...
        return max(range(len(subset)), key=lambda i: subset[i]["end"] - subset[i]["start"])
...
def _cut_teaser(dubbed_video_path, start, dur, out_path, overlay_text=None, output_dir="workspace"):
...
def generate_teasers(dubbed_video_path, segments, captions, output_dir="workspace"):
    ...
    for platform, spec in PLATFORM_SPECS.items():
        ...
        log("TEASER", f"[{label}] strategy={strategy} start={start:.1f}s dur={dur:.1f}s")

        ok = _cut_teaser(dubbed_video_path, start, dur, out_path, output_dir=output_dir)
        if ok:
...
def generate_teaser(dubbed_video_path, segments, output_dir="workspace",
                    min_dur=6.0, max_dur=10.0):
